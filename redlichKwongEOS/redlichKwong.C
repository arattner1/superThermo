...
Foam::redlichKwong<Specie>::redlichKwong(Istream& is)
:
    Specie(is),
    Tc_(readScalar(is)),
    Vc_(readScalar(is)),
...
{
    is.check("redlichKwong<Specie>::redlichKwong(Istream& is)");
}


template<class Specie>
Foam::redlichKwong<Specie>::redlichKwong
(
...
)
{
    ...
    os.check
    (
        "Ostream& operator<<(Ostream& os, const redlichKwong<Specie>& st)"
    );
    return os;
}
